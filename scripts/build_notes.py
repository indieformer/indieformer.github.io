...
TEMPLATE_VERSION = 7  # bump when the post or index template changes materially
...
<header class="site-nav" data-shared-nav></header>
...
<footer class="site-footer" data-shared-footer></footer>
...
<script>
  (function () {{
    ...
    if (!sub) return;
    var done = false;
    function reveal() {{ if (done) return; done = true; sub.classList.add('form-ready'); }}
    function watch(f) {{
      try {{ f.addEventListener('load', reveal); }} catch (e) {{}}
      if (window.ResizeObserver) {{
        try {{
          var ro = new ResizeObserver(function () {{
            if (f.offsetHeight > 40) {{ ro.disconnect(); reveal(); }}
          }});
          ro.observe(f);
        }} catch (e) {{}}
      }}
